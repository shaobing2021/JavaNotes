...
* 创建contoller
* 提前创建service用于注入
* 通过controller类模板获取需要注入的属性
* 获取注入的属性名字，根据名字+set拼接成setXXX方法，同时传入参数，获取对应方法
* 调用method.invoke，给属性赋值
...
//        System.out.println(service);  di.UserService@506e6d5e
        System.out.println(controller.getUserService());//di.UserService@506e6d5e
    }
}
...
* 创建controller
* 通过controller类模板获取所有属性
* 遍历属性看其是否有autowired注解
* 如果有该注解，则通过获取该属性的实例
* 然后通过set方法进行注入
...
//                System.out.println(type);//class di.UserService
...
spring两个核心：xml配置和注解(autowired)

* 通过xml或者annotation或者config都可以给类赋值，bean包含的信息(autowired用于给属性装配)
* bean的定义信息都存放在BeanDefinition中
* beanDefinitionReader就可以读取beandefinition，在确定读取完信息之前可以对配置进行一些修改，读取到一堆bean的定义信息。
* 从bean对象实例之前以及之后有processor可以处理一些事情，同时还有监听器(观察者模式)。
  * 为什么要这么多processor：可以扩展
* 然后就是实例化，实例化就是通过反射的方式获取指定的对象

![](https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=2746420487,3902382017&fm=26&gp=0.jpg)
