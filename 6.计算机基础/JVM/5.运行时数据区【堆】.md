...
- 首先对虚拟机进行配置，如图 Run-Edit configurations
  ![image-20200725231723365](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173657.png)
- 在jdk目录，我的是/Library/Java/JavaVirtualMachines/jdk1.8.0_171.jdk/Contents/Home/bin下找到jvisualvm 运行（或者直接终端运行jvisualvm），查看进程，可以看到我们设置的配置信息
  ![image-20200725231746024](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173658.png)
...
- JDK 8以后： 新生区+养老区+元空间
  ...
  - Tenure generation Space：         ==Old/Tenure==
  - Meta Space：                 ==Meta==
    ![image-20200725231822658](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173659.png)
...
![image-20200726101838267](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173700.png)
...
```java
/**
 * -Xms600m -Xmx600m  VM options中设置堆内存空间大小，内存设置小一点，尽早爆出堆内存溢出问题
 */
...
1. jps工具：查看当前系统中有拿些java进程
2. jmap工具：查看某时刻堆内存占用情况  jmap - heap 进程id
3. jconsole工具：图形界面，多功能监测工具，可以连续监测
4. VisualVM：抓取快照分析哪块内存占用量大
...
public class HeapInstanceTest {
    ...
    main(){
    	byte[] array = new byte[1024*1024*10];//一个byte一个字节，1024字节1k，该为10M
    }
}
```

**对应堆空间分配过程**
![image-20200726102644043](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173701.png)
...
- Full GC触发机制
  ...
  - 说明：Full GC 是开发或调优中尽量要避免的，这样暂停时间会短一些
     ![image-20200726102841415](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173702.png)
...
![image-20200726102933864](https://gitee.com/shaobing2021/typora/raw/master/img/20200727173703.png)
...
- **老年代防止长生命周期对象，通常都是从Survivor区域筛选拷贝过来的Java对象。当然，也有特殊情况，我们知道普通的对象会被分配在TLAB上，如果对象较大，JVM会试图直接分配在Eden其他位置上；如果对象他太大，完全无法在新生代找到足够长的连续空闲空间，JVM就会直接分配到老年代**
- **当GC只发生在年轻代中，回收年轻对象的行为被称为MinorGC。当GC发生在老年代时则被称为MajorGC或者FullGC。一般的，MinorGC的发生频率要比MajorGC高很多，即老年代中垃圾回收发生的频率大大低于年轻代**

> 作者：will A
> 链接：https://juejin.im/post/5ecf7488e51d4578615adae0
> 来源：掘金
