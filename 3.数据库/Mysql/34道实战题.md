...
##该题1.求出每个部门平均薪水 2.联合求出每个部门平均等级 3.
...
join
	salgrade s
	###此处如果不加on，那么该表存在多个grade 为3，而本意是想找到t表对应的grade
on
	t.avgsal between s.losal and s.hisal
where
###这里个人一开始写复杂了，将后select当成一张表继续join，而实际不需要join
###这里不能使用order by limit，因为你不知道最低等级对应几个部门，只能先找到最低的部门
...
from
	emp a
	###如果此处不用左连接，那么b empno有一个领导不存在，不会显示这条数据，只显示b所有数据
left join
	emp b
on
	a.mgr = b.empno;
...
from emp e
join dept
where hiredate < (select hiredate from emp where e.mgr = emp.deptno)
...
join (select count(*) as count ,deptno from emp group by deptno) as t
on t.deptno = deptno
where e.job = 'clerk'
...
###不可以使用下，因为是要求最低薪资大于1500的，而不是先薪资大1500的工作
select * from emp where emp.sal > 1500 group by job;
+-----------+----------+
| job       | count(*) |
+-----------+----------+
| ANALYST   |        2 |
...
薪资大于3000的
###其实应该拆解成两部：第一步员工及领导关系，然后领导这边薪资大于3000，做自连接
select e.ename e.empno,e.mgr
from emp e
where e.mgr in (select e.empno from emp e join (select mgr from emp) as ton t.mgr = e.empno where e.sal>3000)
...
A左连接B，A所有数据都将显示，同时显示满足条件的B

A右边连接B，B所有数据都将显示，同时显示满足条件的A

A join B,其实就是inner join，取两个交集

> 复制表

```
CREATE table newSalgrade as (SELECT * from SALGRADE);
insert into newSalgrade VALUES (6,0,1400)
```

* join on a between x and x
* join on a.x = b.x 
* 后面可以接where，应该先将两张表on连接成一张表，再where，否则直接a.x=xx

## Group by

> 思考

[mysql group by执行原理 - 绿色star - 博客园 (cnblogs.com)](https://www.cnblogs.com/newjiang/p/12841570.html)

```
第六题中如果这样去查会报错，原因是e join d之后，d里面的列只有一行，但是对应e里面有多行数据，所以报错
> 1055 - Expression #4 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'thirtyFour.e.MGR' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
SELECT d.*,e.MGR
from EMP as e
JOIN DEPT as d
...
* 自连接。结合not in
* max，只能取得最大值，但是没法取得对应谁是max
* 使用order by xx limit1 ，可以取得最大值，及对应数据

## 其他

* order by 后面不能起别名

* source xxx.sql 直接加载数据
* in ()如果in 后面存在null，一定要排除，否则会出问题
* Timestampdiff()

* Ifnull (xx,0)用法熟悉

* having 和where区别：where是先过滤在分组，而having必须与group by结合使用，找出特定分组

* 使用count的时候别乱使用count(*)，因为需要计算的是人数，通过count(e.name)，但是可能e.name是null，

  所以本来返回0，实际导致返回1

* right join要学会使用，因为有时候要返回某一边全部数据，比如全部门，因为等号中可能存在空

* group by d.xxx，那么select 时也要select d.xx及对应聚合值